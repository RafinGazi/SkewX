...
This section reports the inferred sex chromosome karyotype for this individual, based on sequencing coverage ratios from mosdepth 1Mb windowed output. The approach is inspired by BeXY (Caduff et al. 2024, Genome Biology & Evolution), which showed that rounding the chrX/autosome and chrY/autosome coverage ratios to the nearest 0.5 reliably infers integer copy counts for each sex chromosome.

Autosomes chr18 and chr21 are used as the coverage baseline. PAR1, PAR2 and the centromere are excluded from arm-level calculations to avoid artefacts from repeat-rich and pseudoautosomal regions (Nurk et al. 2022, Altemose et al. 2022).

### Karyotype call
```{r}
#| label: karyotype-summary
#| echo: false
#| message: false
#| warning: false
...
# summary table
kary %>%
    select(individual, karyotype, karyotype_conf, x_copies, y_copies,
           chrX_ratio, chrY_ratio, auto_mean_cov) %>%
    gt() %>%
    tab_header(
        title    = md("**Inferred Karyotype**"),
        subtitle = "Coverage ratio method (BeXY-inspired rounding)"
    ) %>%
    cols_label(
        ...
        x_copies       = "X copies",
        y_copies       = "Y copies",
        chrX_ratio     = "chrX/Autosome",
        chrY_ratio     = "chrY/Autosome",
        auto_mean_cov  = "Autosomal mean coverage"
    ) %>%
    tab_style(
        style = cell_fill(color = "#d4edda"),
        locations = cells_body(
            columns = karyotype,
            rows    = karyotype_conf == "HIGH"
        )
    ) %>%
    tab_style(
    ...
    tab_style(
        ...
        locations = cells_body(
            columns = karyotype,
            rows    = karyotype_conf == "LOW"
        )
    ) %>%
    opt_interactive(use_highlight = TRUE, use_compact_mode = TRUE)
```

### Coverage ratio plot

The bar chart below shows mean sequencing coverage for chr18, chr21, chrX and chrY, each normalised to the autosomal baseline (average of chr18 and chr21). Reference lines indicate expected coverage for 1 copy (0.5x), 2 copies (1.0x) and 3 copies (1.5x). The karyotype call and confidence level are shown in the plot subtitle.

```{r}
#| label: karyotype-plot
#| echo: false
#| fig-cap: "Coverage ratio plot. Bars show mean coverage relative to autosomal baseline."

knitr::include_graphics("ext_karyotype_plot")
```

### Xp vs Xq arm imbalance

This plot compares the mean coverage of the short (Xp) and long (Xq) arms of the X chromosome. Large deviations from balance may indicate structural abnormalities such as deletions.

```{r}
#| label: karyotype-xp-xq
#| echo: false
...
### X chromosome arm analysis

This table shows the masked mean coverage for the Xp and Xq arms separately, after excluding PAR1, PAR2 and the centromere. A large imbalance between arms may indicate a structural abnormality such as an Xp or Xq deletion or an isochromosome.
```{r}
#| label: karyotype-arms
#| echo: false

kary %>%
    select(individual, xp_mean_cov, xq_mean_cov, xp_xq_ratio) %>%
    gt() %>%
    tab_header(title = md("**Xp / Xq Arm Coverage**")) %>%
    cols_label(
        individual  = "Individual",
        xp_mean_cov = "Xp mean coverage",
        xq_mean_cov = "Xq mean coverage",
        xp_xq_ratio = "Xp/Xq ratio"
    ) %>%
    tab_style(
        style = cell_fill(color = "#fff3cd"),
        locations = cells_body(
            columns = xp_xq_ratio,
            rows    = xp_xq_ratio < 0.6 | xp_xq_ratio > 1.67
        )
    ) %>%
    opt_interactive(use_highlight = TRUE, use_compact_mode = TRUE)
```
...
### Flags
```{r}
#| label: karyotype-flags
#| echo: false

if (kary$arm_flags == "none") {
    cat("No arm-level abnormalities detected.")
} else {
    flags <- strsplit(kary$arm_flags, "\\|")[[1]]
    confs <- strsplit(kary$arm_confidence, "\\|")[[1]]
    ...
    data.frame(
        ...
        gt() %>%
        tab_header(title = md("**X Chromosome Arm Abnormalities**")) %>%
        tab_style(
            style = cell_fill(color = "#fff3cd"),
...
{{< downloadthis ext_karyotype_tsv dname="ext_meta_id_karyotype" label="Download karyotype results" icon="file-earmark-arrow-down" type="secondary" class="data-button" id="karyotype_tsv" >}}

## Cohort QC

This section compares this individual's chrX/autosome (RX) and chrY/autosome (RY) coverage ratios against all other samples in the cohort using DBSCAN density-based clustering. Samples that cluster together with the majority of the cohort are flagged as **PASS**, while samples that fall outside the main cluster are flagged as **OUTLIER**.

**What this tells you:** RX and RY together define the sex chromosome dosage signature of a sample. A typical XX female will have RX ≈ 1.0 and RY ≈ 0.0, while an XY male will have RX ≈ 0.5 and RY ≈ 0.5. Clustering across the cohort helps detect mislabelled samples, unexpected karyotypes, or sequencing anomalies — a sample flagged as OUTLIER warrants manual review.

> **⚠️ Note:** DBSCAN requires a minimum number of samples to form a meaningful cluster. With fewer than ~10 samples, all samples may be flagged as OUTLIER. This does **not** indicate a problem with the sample — it simply means the cohort is too small for reliable clustering. The RX and RY values themselves remain valid.
...
cohort_tsv <- "ext_cohort_tsv"
karyotype_tsv <- "ext_karyotype_tsv"
cohort <- read_tsv(cohort_tsv, show_col_types = FALSE)
kary   <- read_tsv(karyotype_tsv, show_col_types = FALSE)
sample_id <- kary$individual[1]
row <- cohort %>%
    filter(individual == sample_id)
if (nrow(row) == 0) {
    cat("This sample was not found in cohort QC results.")
} else {
    cat(sprintf("Sample %s compared against cohort distribution.\n\n", sample_id))
    required_cols <- c("individual", "chrX_ratio", "chrY_ratio", "karyotype", "cohort_qc_flag")
    missing_cols <- setdiff(required_cols, colnames(row))
    if (length(missing_cols) > 0) {
        cat("Missing expected columns in cohort results:", paste(missing_cols, collapse=", "))
    } else {
        row %>%
            select(all_of(required_cols)) %>%
            gt() %>%
            tab_header(
                title = md("**Cohort-level QC Check**"),
                subtitle = "Position of this sample relative to cohort clusters"
            ) %>%
            cols_label(
                individual       = "Individual",
                chrX_ratio       = "RX",
                chrY_ratio       = "RY",
                karyotype        = "Karyotype",
                cohort_qc_flag   = "QC Status"
            ) %>%
            tab_style(
                style = cell_fill(color = "#d4edda"),
                locations = cells_body(rows = cohort_qc_flag == "PASS")
            ) %>%
            tab_style(
                style = cell_fill(color = "#f8d7da"),
                locations = cells_body(rows = cohort_qc_flag != "PASS")
...
## Skew-enhanced phasing

This VCF has been re-phased using X-inactivation skew information.  
Only chrX variants within informative phase blocks are adjusted.

{{< downloadthis ext_skew_phased_vcf 
    dname="ext_meta_id_skew_phased" 
    label="Download skew-phased VCF" >}}

### Skew phasing summary

Variants phased on chrX were adjusted using skew information derived from haplotype imbalance.

A higher flip rate indicates stronger skew and greater confidence in skew-driven phasing.

- **<10%** → weak/no skew  
- **10–20%** → moderate skew  
- **>20%** → strong skew (informative sample)

```{r}
#| echo: false

skew_file <- "ext_skew_metrics"
if (file.exists(skew_file)) {

    skew_metrics <- read.delim(skew_file)

    cat(sprintf("Flip rate: %.2f%%\n\n", skew_metrics$flip_rate))
    skew_metrics %>%
        gt() %>%
        tab_header(title = md("**Skew Phasing Metrics**")) %>%
        cols_label(
            total_variants = "Total chrX variants",
            flipped        = "Variants flipped",
            kept           = "Variants kept",
            skipped        = "Variants skipped",
            flip_rate      = "Flip rate (%)"
        ) %>%
        fmt_number(columns = flip_rate, decimals = 2) %>%
        tab_style(
            style = cell_fill(color = "#fff3cd"),
            locations = cells_body(
                columns = flip_rate,
                rows = flip_rate > 10 & flip_rate <= 20
            )
        ) %>%
        tab_style(
            style = cell_fill(color = "#d4edda"),
    ...
    cat("No skew phasing metrics available.")
}
```
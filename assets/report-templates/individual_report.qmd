...
#| echo: false
#| fig-cap: "Coverage ratio plot. Bars show mean coverage relative to autosomal baseline. Reference lines indicate expected copy number levels."
...
if (kary$flags == "none") {
    cat("No abnormality flags raised for this individual.")
} else {
    flags     <- strsplit(kary$flags,           "\\|")[[1]]
    confs     <- strsplit(kary$flag_confidence, "\\|")[[1]]
    reasons   <- strsplit(kary$flag_reason,     " \\|\\| ")[[1]]
    lits      <- strsplit(kary$flag_literature, " \\|\\| ")[[1]]
    ...
    data.frame(
        ...
        gt() %>%
        tab_header(title = md("**X Chromosome Abnormality Flags**")) %>%
        tab_style(
            style = cell_fill(color = "#fff3cd"),
        ...
        tab_style(
            ...
            locations = cells_body(columns = c(Reason, Literature))
        ) %>%
        opt_interactive(use_highlight = TRUE, use_compact_mode = TRUE)
}
```

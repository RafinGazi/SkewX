...
if (qc_flag == "pass") {
    cat("🟢 PASS — proceeding with full analysis.\n\n")
} else if (startsWith(qc_flag, "skipped")) {
    cat(paste0("🟡 SKIPPED — sample excluded from downstream analysis (", qc_flag, ").\n\n"))
} else if (startsWith(qc_flag, "flagged")) {
    cat(paste0("🔴 FLAGGED — anomaly detected (", qc_flag, "). Manual inspection recommended.\n\n"))
}
```
...
if (qc_flag != "pass") {
    cat(paste0("Skew analysis not performed due to QC status: ", qc_flag, "\n"))
} else { 
    cat("Skew calculations are based on the number of Xa and Xi reads assigned to each haplotype, in each block. These count tables for each sample can be downloaded below.")
}
...
kary %>%
    ...
    { if ("karyotype_conf" %in% names(kary))
        tab_style(.,
            style = cell_fill(color = "#d4edda"),
            locations = cells_body(columns = karyotype, rows = karyotype_conf == "HIGH")
        ) else . } %>%
    { if ("karyotype_conf" %in% names(kary))
        tab_style(.,
            style = cell_fill(color = "#fff3cd"),
            locations = cells_body(columns = karyotype, rows = karyotype_conf == "MEDIUM")
        ) else . } %>%
    { if ("karyotype_conf" %in% names(kary))
        tab_style(.,
            style = cell_fill(color = "#f8d7da"),
            locations = cells_body(columns = karyotype, rows = karyotype_conf == "LOW")
        ) else . } %>%
